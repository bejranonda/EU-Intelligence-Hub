...
async def get_keyword_sentiment(keyword_id: int, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        sentiments = [
            a.sentiment_overall for a in articles if a.sentiment_overall is not None
        ]
        ...
        distribution = {
            "strongly_positive": sum(
                1 for a in articles if a.sentiment_classification == "STRONGLY_POSITIVE"
            ),
            "positive": sum(
                1 for a in articles if a.sentiment_classification == "POSITIVE"
            ),
            "neutral": sum(
                1 for a in articles if a.sentiment_classification == "NEUTRAL"
            ),
            "negative": sum(
                1 for a in articles if a.sentiment_classification == "NEGATIVE"
            ),
            "strongly_negative": sum(
                1 for a in articles if a.sentiment_classification == "STRONGLY_NEGATIVE"
            ),
        }
        ...
        most_positive = (
            max(source_averages.items(), key=lambda x: x[1])
            if source_averages
            else None
        )
        most_negative = (
            min(source_averages.items(), key=lambda x: x[1])
            if source_averages
            else None
        )
...
async def get_sentiment_timeline(
    ...
    try:
            ...
            timeline.append(
                {
                    ...
                    "average_sentiment": (
                        round(trend.average_sentiment, 3)
                    ...
                    "total_articles": (trend.positive_count or 0)
                    + (trend.negative_count or 0)
                    + (trend.neutral_count or 0),
        ...
        if len(timeline) >= 2:
            ...
            if first_sentiment is not None and last_sentiment is not None:
                trend_direction = (
                    ...
                    else "declining" if last_sentiment < first_sentiment else "stable"
                )
                trend_change = (
                    round(
                        (last_sentiment - first_sentiment) / abs(first_sentiment) * 100,
                        1,
                    )
        ...
        return {
            ...
            "timeline": timeline,
            "trend": {"direction": trend_direction, "change_percent": trend_change},
        }
...
async def compare_keyword_sentiment(
    keyword_ids: str = Query(
        ..., description="Comma-separated keyword IDs (e.g., '1,2,3')"
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        ...
        if len(ids) < 2:
            raise HTTPException(
                status_code=400, detail="At least 2 keywords required for comparison"
            )

        if len(ids) > 5:
            raise HTTPException(
                status_code=400, detail="Maximum 5 keywords allowed for comparison"
            )
        ...
        if len(keywords) != len(ids):
            raise HTTPException(
                status_code=404, detail="One or more keywords not found"
            )
        ...
        for keyword in keywords:
            ...
            if articles:
                sentiments = [
                    a.sentiment_overall
                    for a in articles
                    if a.sentiment_overall is not None
                ]
                avg_sentiment = sum(sentiments) / len(sentiments) if sentiments else 0
                ...
                comparison.append(
                    {
                        ...
                        "positive_count": sum(
                            1
                            for a in articles
                            if a.sentiment_overall and a.sentiment_overall > 0.2
                        ),
                        "negative_count": sum(
                            1
                            for a in articles
                        ...
                        "neutral_count": sum(
                            ...
                            and -0.2 <= a.sentiment_overall <= 0.2
                        ),
                    }
                )
            else:
        ...
        comparison.sort(
            key=lambda x: (
                x["average_sentiment"] if x["average_sentiment"] is not None else -999
            ),
            reverse=True,
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error comparing keywords: {str(e)}"
        )


@router.get("/articles/{article_id}/sentiment")
async def get_article_sentiment_details(article_id: int, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        keywords = (
            ...
            .filter(KeywordArticle.article_id == article.id)
            .all()
        )
        ...
        return {
            ...
            "keywords": [
                {"id": kw.id, "keyword_en": kw.keyword_en, "category": kw.category}
                for kw in keywords
            ],
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving sentiment details: {str(e)}"
        )
