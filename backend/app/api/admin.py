...
from app.models.models import KeywordSuggestion, Keyword, KeywordEvaluation, NewsSource, SourceIngestionHistory
@router.get("/sources")
async def list_news_sources(
    only_enabled: bool = Query(False, description="Return only enabled sources"),
    db: Session = Depends(get_db),
    ...
    sources = (
        query.order_by(NewsSource.priority.desc(), NewsSource.created_at.asc())
        .all()
    )
