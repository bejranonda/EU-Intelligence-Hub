    ...
    try:
        ...
        try:
            new_suggestion = KeywordSuggestion(
                ...
                keyword_th=(
                    suggestion.keyword_th.strip() if suggestion.keyword_th else None
                ),
                keyword_de=(
                    suggestion.keyword_de.strip() if suggestion.keyword_de else None
                ),
                keyword_fr=(
                    suggestion.keyword_fr.strip() if suggestion.keyword_fr else None
                ),
                keyword_es=(
                    suggestion.keyword_es.strip() if suggestion.keyword_es else None
                ),
                keyword_it=(
                    suggestion.keyword_it.strip() if suggestion.keyword_it else None
                ),
                keyword_pl=(
                    suggestion.keyword_pl.strip() if suggestion.keyword_pl else None
                ),
                keyword_sv=(
                    suggestion.keyword_sv.strip() if suggestion.keyword_sv else None
                ),
                keyword_nl=(
                    suggestion.keyword_nl.strip() if suggestion.keyword_nl else None
                ),
                category=(
                    suggestion.category.strip() if suggestion.category else "general"
                ),
                reason=suggestion.reason.strip() if suggestion.reason else None,
                contact_email=(
                    suggestion.contact_email.strip()
...
async def get_suggestion(suggestion_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "created_at": suggestion.created_at.isoformat(),
            "updated_at": (
                suggestion.updated_at.isoformat() if suggestion.updated_at else None
            ),
        }
